...
const calculateMinAda = async (
  ...
  const sumAssetNameLengths = assets.reduce(
    (acc: number, a: any) =>
      acc + (a?.length || 0) > 56 ? `${a}`.slice(56).length : 0,
    0,
  );
  ...
  const size =
    6 +
    roundupBytesToWords(
      assets.length * 12 + sumAssetNameLengths + policyIds.size * pidSize,
    );
...
export const sendAssets = async (
  ...
  if (strategy === UTXOStrategy.DEFAULT) {
    ...
    if (
      parseInt(outputs.find((x) => x.unit === 'lovelace')?.quantity || '0') < minAda
    ) {
  ...
  if (adaAmount > 0) {
    ...
    tx.sendLovelace({ address: walletAddress }, adaQuantity);
  }
